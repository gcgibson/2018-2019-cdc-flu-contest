...
for (region in c("National",paste0("Region ",1:10))){
  for (week in c(seq(40,52),seq(20))){
    if (week <=9){
      data_week <- paste0("0",week)
    }else{
      data_week <- week
    }
    test_dat <- data.frame(Region=region,season_week = week,Incidence=data[data$region==region & data$epiweek==paste0(2015,data_week) & data$lag==0,]$weighted_ili)
    ...
    truth <- lag_df[lag_df$Region==region & lag_df$week==paste0(2015,week),"X0" ]
