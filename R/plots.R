...
p <- ggplot(data.frame(season_week=1:53,X0=lm.predict), aes(x=season_week,y=X0)) +geom_line()
p <- p+ geom_point(data=lag_df0,aes(x=season_week,y=X0),alpha=.1,col="cornflowerblue") + theme_bw()
p
